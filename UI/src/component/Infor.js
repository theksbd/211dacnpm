...
function Infor(props) {
    ...
    for (var i=0; i<data.length; i++){
        if(location.state.productName==data[i].Product_Name){
            var temp =true
            var temp1=true
            var temp2=true
    ...
    const [rominfor, setRominfor] = useState(location.state.specifications.rom)
    const [priceinfor, setPriceinfor] = useState(location.state.newPrice)
    const [imginfor, setImginfor] = useState(location.state.image)
